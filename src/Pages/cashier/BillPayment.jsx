...
const BillPayment = () => {
  ...
  const fetchLabTests = async (patientId) => {
    try {
      const response = await apiUtility.get(`/labtest/getLabTest/${patientId}`);
      if (response.status) setLabTests(response.data);
    } catch (err) {
      setError("Unable to fetch lab test records");
    }
  ...
  const handleUpdateStatus = async () => {
    try {
      const updatedBill = { PatientID: selectedPatient, Status: status };
      await apiUtility.put("/bill/updateStatus", updatedBill);
      fetchBillData();
    } catch (err) {
  ...
  return (
    <>
      <Box mx={5} my={3}>
        ...
        {labTests.length > 0 && (
          <>
            <Typography variant="h6" mt={3}>Lab Test Records</Typography>
            ...
            <Typography mt={2} variant="h6">Total Amount: {totalAmount}</Typography>
          </>
        )}
